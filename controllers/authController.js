...
async function autenticar(req, res) {
  ...
  if (usuario.estaBloqueado()) {
    await registrarLog(req, { usuario: usuario._id, oficina: usuario.oficina, acao: 'conta_bloqueada' });
  ...
  if (!senhaCorreta) {
    ...
    await usuario.save();
    await registrarLog(req, { usuario: usuario._id, oficina: usuario.oficina, acao: 'login_falha', detalhes: 'Senha incorreta' });
    req.session.mensagemErro = 'E-mail ou senha inválidos.';
    return res.redirect('/login');
  }
  ...
  await registrarLog(req, { usuario: usuario._id, oficina: usuario.oficina, acao: 'login_sucesso' });
...
async function sair(req, res) {
  if (req.usuario) {
    await registrarLog(req, { usuario: req.usuario._id, oficina: req.usuario.oficina, acao: 'logout' });
  }
...
async function redefinirSenha(req, res) {
  ...
  await registrarLog(req, { usuario: usuario._id, oficina: usuario.oficina, acao: 'senha_recuperada' });
